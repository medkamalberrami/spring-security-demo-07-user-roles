...
public class DemoSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		auth.inMemoryAuthentication().withUser(users.username("Kamal").password("test123").roles("EMPLOYEE","ADMIN","MANAGER"))
				.withUser(users.username("Med").password("med123").roles("EMPLOYEE","MANAGER"))
